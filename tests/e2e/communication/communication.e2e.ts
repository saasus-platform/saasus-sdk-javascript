...
import { getCommunicationStories, verifyMethodCoverage } from './stories';
...
async function runCommunicationE2E(): Promise<void> {
  loadEnv();

  console.log('🚀 Starting SaaSus Communication API E2E Tests');
  console.log('============================================');
  ...
  const stories = getCommunicationStories();
...
runCommunicationE2E().catch(error => {
  console.error('Failed to execute communication E2E tests', error);
  process.exitCode = 1;
});
