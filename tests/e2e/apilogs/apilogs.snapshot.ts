...
import { getApiLogStories } from './stories';
...
async function runApiLogsSnapshot(): Promise<void> {
  loadEnv();
  const options = parseArgs();
  applySnapshotMode(options.mode, options.outputDir);

  console.log('🚀 Starting SaaSus ApiLogs API Snapshot Tests');
  console.log('============================================');
  ...
  const stories = getApiLogStories(options.stories);
...
runApiLogsSnapshot().catch(error => {
  console.error('Failed to execute ApiLogs snapshot tests', error);
  process.exitCode = 1;
});
