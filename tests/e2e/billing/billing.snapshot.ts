...
import { createBillingE2EClient, getBillingMethods } from './client';
import { ensureStripeTestPreconditions } from './state';
import { getBillingStories } from './stories';
...
async function runBillingSnapshot(): Promise<void> {
  loadEnv();
  const options = parseArgs();
  applySnapshotMode(options.mode, options.outputDir);

  console.log('🚀 Starting SaaSus Billing API Snapshot Tests');
  console.log('==============================================');
  ...
  await ensureStripeTestPreconditions(client);
  const engine = new E2EEngine(client, getBillingMethods());
  const stories = getBillingStories(options.stories);
...
runBillingSnapshot().catch(error => {
  console.error('Failed to execute billing snapshot tests', error);
  process.exitCode = 1;
});
