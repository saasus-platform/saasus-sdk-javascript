...
import { validateStoryExecution } from './validation';
...
async function runIntegrationSnapshot(): Promise<void> {
  loadEnv();
  const options = parseArgs();
  applySnapshotMode(options.mode, options.outputDir);

  console.log('🚀 Starting SaaSus Integration API Snapshot Tests');
  console.log('================================================');
  ...
  const stories = getIntegrationStories(options.stories);
  ...
  try {
    ...
    if (results.length > 0) {
      const validationResult = validateStoryExecution(
        results[0].story.name,
...
runIntegrationSnapshot().catch(error => {
  console.error('Failed to execute integration snapshot tests', error);
  process.exitCode = 1;
});
