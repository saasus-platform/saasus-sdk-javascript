...
import { createPricingE2EClient, getPricingMethods } from './client';
import { getPricingStories } from './stories';
...
async function runPricingSnapshot(): Promise<void> {
  loadEnv();
  const options = parseArgs();
  applySnapshotMode(options.mode, options.outputDir);

  console.log('🚀 Starting SaaSus Pricing API Snapshot Tests');
  console.log('==============================================');
  ...
  // Disable fail fast to execute all stories (use both env keys for compatibility)
  process.env.E2E_FAIL_FAST = 'false';
  process.env.FAIL_FAST = 'false';

  const client = createPricingE2EClient();
  const engine = new E2EEngine(client, getPricingMethods());
  const stories = getPricingStories(options.stories);

  const results = await engine.executeStories(stories);
  engine.generateReport(results);

  console.log('\n📊 Snapshot execution finished. Generated snapshots for all stories.');
}

runPricingSnapshot().catch(error => {
  console.error('Failed to execute pricing snapshot tests', error);
  process.exitCode = 1;
});
